// This script extracts pages into separate InDesign documents so that HTML files can be made from each

//Particularly useful for InDesign to iBooks
// Modified by CJ
// with acknowledgement to Loïc Aigon
...
function extractdlg()
{
	...
	var dlg = app.dialogs.add({name : "Pages and IDML Extractor 1.1a www.loicaigon.com+CJ"});
	with(dlg)
	{
		...
		with(firstclmn)
		{
			...
			with(firstrow)
			{
				...
				with(clmn1)
				{
					...
					row1.staticTexts.add({staticLabel : "Carefully choose pages to extract..."});
					...
					with(row2)
					{
						...
						with(r2c2)
						{
							...
							var pgStart = r2c2r1.realEditboxes.add({editValue:pageinfos()[1], minWidth: 80});
						}
						...
						with(r2c4)
						{
							...
							var pgEnd = r2c4r1.realEditboxes.add({editValue:pageinfos()[2], minWidth:80});
						}
...
// 			}
			...
			for(w=0; w<W;w++)
			{
				...
				else
				{
					...
					var exporthtmlname = "_Pg"+pgStart.editValue+"_to_Pg_"+pgEnd.editValue +".html";
				}
...
// 				var mySelection = app.activeDocument.parentStory;
// 				if(mySelection.overflows == true){
...
// 				var exportidml = doc.save(File(saveextractfolder + exportidmlname));
                var exporthtml = doc.exportFile("HTML",File(saveextractfolder + exporthtmlname));
				exportdoc.close(SaveOptions.no);
// 				if(sepbox.checkedState==true && w<(pgEnd.editValue-pgStart.editValue))
// 				{
					app.open(File(docfile));
					alert("\nCompleted and saved: "+saveextractfolder+exportdocname+"\n\nAnd Exported "+saveextractfolder+exporthtmlname);
// 				}
			}
